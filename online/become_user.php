...
if ($userId < 1 ) { 
  ...
  exit();
    ...
    $query = "SELECT * FROM users  WHERE userId =" . $_GET['u'];
$db = IOC::make('database', array());
	list($affect_rows, $datad) = $db->selectquerys($query);
    // $result = mysql_query($query);
       // if (!$result) die ("SQL error on users : " . mysql_error());
...
$query .= "AND Status like '%Ready for Collection%' ";
// echo "SQL: $query<p>";
$db = IOC::make('database', array());
list($affect_rows, $datad) = $db->selectquerys($query);
$no_records = count($datad);
// echo "No records: $no_records<p>";

// Get page number and set variables

if (!(isset($_GET['page']) && ($_GET['page'] / 1 == $_GET['page']) && $_GET['page'] != 0)) {
  $no_page = 1;
}
    ...
    <div class="col-md-2">&nbsp;</div>
        <div class="col-md-8">
          <ul class="nav nav-pills" style="position:relative;left:-60px;">
            <li style="background-color:rgb(238,238,238);-moz-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; -khtml-border-radius: 4px;"><a href="become_user.php?u=<?php echo $_GET['u']; ?>&type=global">Global</a></li>
            <li style="background-color:rgb(238,238,238);-moz-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; -khtml-border-radius: 4px;"><a href="become_user.php?u=<?php echo $_GET['u']; ?>&type=ua">Under Assessment</a></li>
            <li  style="background-color:rgb(238,238,238);-moz-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; -khtml-border-radius: 4px;"><a href="become_user.php?u=<?php echo $_GET['u']; ?>&type=rc">Ready for Collection</a></li>
            <li style="background-color:rgb(238,238,238);-moz-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; -khtml-border-radius: 4px;"><a href="become_user.php?u=<?php echo $_GET['u']; ?>&type=ar">Accepted and Rejected Jobs</a></li>
            <li style="background-color:rgb(238,238,238);-moz-border-radius: 4px;-webkit-border-radius: 4px;border-radius: 4px; -khtml-border-radius: 4px;"><a href="become_user.php?u=<?php echo $_GET['u']; ?>&type=q">Quoted Jobs</a></li>
          </ul>
...
<?php
         ...
         $query = "SELECT * FROM users u INNER JOIN jobinfoweb j ON (u.customer = j.customer) WHERE u.userId = '" . $_GET['u'] . "' ";
    
        if(isset($_GET['type'])){
            $type =  $_GET['type'];
            if($type == 'rc'){
                $query .= " AND Status='Ready for Collection' ";
            }else if($type == 'ua'){
                $query .= " AND Status='Under Assessment' ";
            }else if($type == 'q'){
  ...
  //var_dump($datad);
    if ($affect_rows > 0) {
      ...
      foreach ($datad as $row) {
        ...
        echo "<tr>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'></a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_jobno . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_date . "</a></td>";

        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_make . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_model . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_serial . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_amount . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_tax . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_total . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>" . $job_quote_status . "</a></td>";
        echo "<td><a href='quote.php?qid=" . $job_jobno . "&icon=3'>Accepted</a></td>";
    ...
    ?>
        ...
        <? php// include "inc_table_collection_nav.php"; ?>
