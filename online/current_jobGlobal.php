...
$query .= "WHERE userId= '" . $_SESSION['userId'] . "' AND QuoteActionRequired = 'Quote'";
      ...
      <div class="row">
        ...
        <div class="col-md-8">
          ...
          <ul class="nav nav-pills" style="position:relative;left:-60px;">
            ...
            <li class="active"><a href="current_jobGlobal.php">Quoted Jobs</a></li>
          </ul>
...
?>
		...
		foreach ($datad as $row)
      {
        ...
        if($_SESSION['accountType'] == 'admin'){
          $query = "SELECT * FROM users u INNER JOIN jobinfoweb j ON (u.customer = j.customer) WHERE u.linkedTo = '" . $_SESSION['adminId'] . "' AND Status = 'Quote' ";

        }else{
          $query = "SELECT * FROM users u INNER JOIN jobinfoweb j ON (u.customer = j.customer) WHERE u.userId = '" . $_SESSION['adminId'] . "' AND Status = 'Quote' ";
    ...
    if (count($datad) > 0) {
	  ...
	  $delivery_address       = $row['DeliveryAddress'];
		$workshop       = $row['Workshop'];
          ...
          echo "<tr>";
          $counter = $j + $no_start;
          echo "<td><a href='quote.php?qid=" . $job_jobno . "'>" . $i++ . ".</a></td>";
