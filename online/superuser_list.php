...
<div class="row">
    <div class="col-md-2">&nbsp;</div>
      ...
      <h1 class="form-signin-heading">Below is a list of all the admin accounts and the accounts assigned to them</h1>
      <br>
      <?php
	$sql = "SELECT * FROM users WHERE userLimit=5";
	$db = IOC::make('database', array());
	...
	if($affect_rows == 0)
		...
		foreach ($data as $result)
      {
				...
				$inner_query = "SELECT * FROM users WHERE linkedTo=".$result['userId'] ." ORDER BY loginName ASC";
				//print_r("<pre>");var_dump($data); die();
                $db = IOC::make('database', array());
				list($affect_rows, $data) = $db->selectquerys($inner_query);
				
                $str = "";
				
					foreach ($data as $inner_row)
					{
						$str .= $inner_row['loginName']. ", ";
					}
					$str = rtrim($str, ', ');
				
				
				echo '<p>
				<a href="superuser_deleteAdmin.php?aid='.$result['userId'].'"><button type="button" class="btn-xs btn-danger">Delete Admin Account				                </button></a>
				<a href="superuser_editAdmin.php?aid='.$result['userId'].'"><button type="button" class="btn-xs btn-warning">Edit</button></a>
				<a href="#">'.$result['loginName'].'</a> => '.$str.'</p>';
