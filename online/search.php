...
$form ="<div align='left' >
		<br>
		<form name=getDate method='post'>
		<table>
			<input type='hidden' name='option' value='report' />
			
			<label>Job:</label>
			<input name='Job' value='".@$Job."'  />&nbsp;&nbsp;&nbsp
			<label>Customer Name:</label>
			<input name='CustomerName' value='".@$CustomerName."' />&nbsp;&nbsp;&nbsp
			<label>Quote Date:</label>
			<input name='QuoteJobDate' value='".@$QuoteJobDate."' />&nbsp;&nbsp;&nbsp
			<input type='button' name='date' onClick=check(getDate) value='Search' />
		</table>
		</form>
		<script>
			function check(Frm){
				if(Frm.Job.value.length == 0 | Frm.CustomerName.value.length == 0 | Frm.QuoteJobDate.value.length == 0){
								//alert('Please Enter a Search Criteria');
								//Frm.Job.focus(); return false;
						  	}
				Frm.submit();
			}
		</script>
	...
	list($affect_rows, $datad) = $db->selectquerys($sql);
		 ...
		 foreach ($datad as $row)
      {
		 ...
		 $sql ="SELECT * FROM JobUpdateWeb
					...
					$db = IOC::make('database', array());
					list($affect_rows, $datad) = $db->selectquerys($sqll);
					$answer="visible";
