...
echo dirname(dirname(__FILE__)) . "/online/_autoload.php";
die();
require_once dirname(__FILE__)."/libraries/PHPMailer-master/PHPMailerAutoload.php"; // include PHP mailer
require_once (dirname(dirname(__FILE__)) . "/library/stdinc.php");
require_once (dirname(dirname(__FILE__)) . "/online/include/config.php");
require_once (dirname(dirname(__FILE__)) . "/online/_autoload.php");
...
echo '<br/>
	<form method="post" name="getOrderNumber" enctype="multipart/form-data">
		...
		<input name="orderNum" type="text" />
                <br>
                <br>
                <label>Upload Purchase Order File <small><em>( *.pdf )</em></small>:</label>
		<input name="orderFile" type="file" />
                <br>
                <input type="hidden" name="sendEmail" value="1" />
		<a href="javascript:history.go(-1)"><button type="button" class="btn btn-warning btn">Back</button></a> &nbsp; <input name="send" type="button" class="btn btn-primary btn" value="Send" onclick="check(this.form)" />
	</form>
	<script>
		function check(frm){
...
<?php
// accept.php?Job=JN167982&Customer=&AccountContactName=&AccountContactEmail=bilkiesm@mustek.co.za&AccountContactTel=&AccountContactCell=&SalesContactName=&SalesContactEmail=&SalesContactTel=&SalesContactCell=
$conn = new PDO("mysql:host=localhost;port=3306;dbname=$db", $dbUser, $dbPassword);
$job_id = (isset($_GET['job']) ? $_GET['job'] : 'JN173701');
//echo $job_id;
$sql= "SELECT * FROM JobPurchaseOrders";
$stmt = $conn->query($sql); 
...
print_r("<pre>"); var_dump($trp);
echo "<br/>";
print_r("<pre>"); var_dump($trps); die();
if ( isset($_POST['orderNum']) ){
    ...
    if (!isset($_FILES['orderFile']) || !isset($_FILES['orderFile']['name'])) {
        echo "<script>alert('No file selected');</script>";
    }
    else if (strpos(strtolower($_FILES['orderFile']['name']), ".pdf") === FALSE) {
        echo "<script>alert('You may upload only PDF files');</script>";
    }
    else {
        try {
            ...
            if (!move_uploaded_file($_FILES['orderFile']['tmp_name'], $filename)) {
                ...
                $writeUrl = 'acceptWrite.php?answer='.$_POST['orderNum'].'&Job='.$_GET['Job'].'&FaultDescription='.
                        $_GET['FaultDescription'].'&WorkDone='.$_GET['WorkDone'].'&Item='.$_GET['Item'].'&QuoteTotal='
                        .$_GET['QuoteTotal'].'&QuoteTotalTax='.$_GET['QuoteTotalTax'].'&QuoteGrandTotal='
                        .$_GET['QuoteGrandTotal'].'&Accessories='.$_GET['Accessories'].'&Customer='.$_GET['Customer']
                        .'&AccountContactName='.$_GET['AccountContactName'].'&AccountContactEmail='.$_GET['AccountContactEmail'].'&AccountContactTel='
                        .$_GET['AccountContactTel'].'&AccountContactCell='.$_GET['AccountContactCell'].'&SalesContactName='
                        .$_GET['SalesContactName'].'&SalesContactEmail='.$_GET['SalesContactEmail'].'&SalesContactTel='
                        .$_GET['SalesContactTel'].'&SalesContactCell='.$_GET['SalesContactCell'].' ';

                // check if someone submitted the file
                if (isset($_POST['sendEmail']) && $_POST['sendEmail'] == 1) {
                    $mail = new PHPMailer();

                    $mail->isHTML(true);

                    $htmlMessage = "<br><b>Job Number:</b> " . $_GET['Job'] . "<br><br><b>Customer:</b> ".$_GET['Customer']."<br><br><b>Account Contact Email:</b> {$_GET['AccountContactEmail']}<br><br>Quote has been ACCEPTED by client.<br/><br/>Regards<br/>Partserve Online";

                    $mail->setFrom('info@partserve.co.za', 'Partserve');
                    $mail->addAddress(@$_GET['AccountContactEmail']);
                    $mail->addAddress('csteam@partserve.co.za');   //csteam@partserve.co.za
                    $mail->addBCC('onlinejt@partserve.co.za','Partserve');
                    $mail->addBCC("gavin@itnt.co.za");

                    if (file_exists($purchaseOrder->file)) {
                        $mail->addAttachment($purchaseOrder->file);
                    }
