    ...
    if ($dateAudit != NULL) {
        try {
            ...
            $fields = array(
                ...
                array(
                    'Log Date', friendly_date($dateAudit->logdate, 'Y-m-d H:i:s')
                ),
                array(
                    'Quote Date', $dateAudit->quotedate == '0000-00-00 00:00:00' ? 'N/A' : $dateAudit->quotedate
                ),
                array(
                    'Accept Date', friendly_date($dateAudit->acceptdate, 'Y-m-d H:i:s')
                ),
                array(
                    'Invoice Date', friendly_date($dateAudit->invoicedate, 'Y-m-d H:i:s')
                ),
                array(
                    'Store Date', friendly_date($dateAudit->storedate, 'Y-m-d H:i:s')
                ),
                array(
                    'Collection Ref', friendly_date($dateAudit->collectionref, 'Y-m-d H:i:s')
                )
            );
        ...
        ?>
          ...
          <tr>
            ...
            <tr>
                <td>&nbsp;</td>
                <td>
                  <a href='accept.php?Job=<?php echo $Job ?>&Customer=<?php echo $Customer ?>&AccountContactName=<?php echo $AccountContactName ?>&AccountContactEmail=<?php echo $QouteAccountEmail; ?>&AccountContactTel=<?php echo $AccountContactTel ?>&AccountContactCell=<?php echo $AccountContactCell ?>&SalesContactName=<?php echo $SalesContactName ?>&SalesContactEmail=<?php echo $SalesContactEmail ?>&SalesContactTel=<?php echo $SalesContactTel ?>&SalesContactCell=<?php echo $SalesContactCell ?>' id='accept' <?php echo @$answer ?>><button class="btn btn-success">Accept</button></a>
                  &nbsp;
			...
			<tr>
               <td colspan=2 valign='top'><strong>PROFORMA INVOICE</strong></td>
			...
			<td></td>
                <td >
                  ...
                  <button style="padding: 5px" class="download btn btn-success">Download PROFORMA INVOICE</button>
					...
					<input  style="	width: 400px;
							...
							type="email"  
							placeholder="Please enter the email address to send the proforma invoice to" 
							name="email"
							
							id="email"
					/>
					
					<a class="email-invoice" style="padding: 5px" href="#"><button class="btn btn-info">Email PROFORMA INVOICE</button></a>                
				    </div>
				  </td>
             </tr>
            <?php } ?>
            ...
            <tr>
              ...
              foreach ($dateAuditList as $date){ 
                ...
                <tr style="display: none;">
                    ...
                    <p><strong>Received from Customer:</strong> <?= friendly_date($date->logdate) ?></p>
                          <p><strong>Date Quoted</strong>: <?= friendly_date($date->quotedate) ?></p>
                          <p><strong>Date Quote Accepted:</strong> <?= friendly_date($date->acceptdate) ?> </p>
                          <p><strong>Date ready for collection:</strong> <?= friendly_date($date->invoicedate) ?></p>
                          <p><strong>Date when unit was collected / delivered back to customer:</strong> <?= friendly_date($date->storedate) ?></p>
...
<!--           <tr>
	...
	<script>
		$(document).ready(function(){
			$('.email-invoice').click(function(a){
				a.preventDefault()
				var $button = $(this);
				$('.hidden-input').removeClass('hidden')
				$button.removeClass('email-invoice').addClass('send-invoice').bind("click", function(a){
					a.preventDefault()
					var $sendButton = $(this);
					    var email = $('#email').val()
						if(email != ''){
							$sendButton.find('button').html('Sending...')
							$.ajax({
								url: '<?php echo 'http://'. $_SERVER["HTTP_HOST"]. '/online/invoice.php?qid='.$Job ;?>',
								dataType: 'html',
								method: 'POST',
								data: {email: email, send_invoice: true},
								success: function(a){
							...
							alert('Please enter the email address you want to send the proforma invoice to!');
						}
						
				}).find('button').html('Send email')
			...
			$('button.download').click(function(){
				$.ajax({
					...
					data: {download: true},
					success: function(a){
					window.location.href = "<?php echo 'http://'. $_SERVER["HTTP_HOST"]. '/online/invoices/'.$Job.'.pdf';?>"
					}
				})
			})
		})
