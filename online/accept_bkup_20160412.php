...
echo '<br/>
	<form method="post" name="getOrderNumber" enctype="multipart/form-data">
		...
		<input name="orderFile" value="" type="file" />
                ...
                <input  type="email" name="ccEmailAddress" value= "" required="" size="37"/>
                <br><br>
...
//                        }
			...
			if (frm.ccEmailAddress.value == "") {
                            alert ("Please provide your email address!")
			    frm.ccEmailAddress.setFocus();
			    return false;
                        }
			
			var re = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
			if ( re.test(frm.ccEmailAddress.value) == false) {
...
//    Check the pdf it has not be uploaded
    if (isset($_FILES['orderFile']) && isset($FILES['orderFile']['name']) == null) {
        ...
        $writeUrl = 'acceptWrite.php?answer='.$_POST['orderNum'].'&Job='.$_GET['Job'].'&FaultDescription='.$_GET['FaultDescription']
            ...
            .$_GET['SalesContactName'].'&SalesContactEmail='.$_GET['SalesContactEmail'].'&SalesContactTel='.$_GET['SalesContactTel'].'&SalesContactCell='.$_GET['SalesContactCell'].'&File='.$filename.'&pagename=accept';
    ...
    if(isset($_FILES['orderFile']) && isset($FILES['orderFile']['name']) != null && strpos(strtolower($_FILES['orderFile']['name']), ".pdf") === FALSE) {
        echo "<script>alert('Only pdf files are acceptable for upload.');</script>";
    }
    else {
        try {
