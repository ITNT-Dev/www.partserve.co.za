...
$query .= "AND j.job IN (SELECT job FROM JobUpdateWeb WHERE Customer = users.customer) ";
    ...
    <div class="col-md-2">&nbsp;</div>
        <div class="col-md-8">
          ...
          <ul class="nav nav-pills" style="position:relative;left:-60px;">
            ...
            <li class="active"><a href="closedJobsGlobal.php">Accepted and Rejected Jobs</a></li>
        ...
        <div class="col-md-2">
          ...
          <div class="pull-right">
            <a href="logout.php"><button type="button" class="btn btn-success">Logout&nbsp;<?php
echo $_SESSION['name'];
?></button></a>
...
if($_SESSION['accountType'] == 'admin'){
    $query = "SELECT * FROM users u INNER JOIN jobinfoweb j ON (u.customer = j.customer) WHERE u.linkedTo = '" . $_SESSION['adminId'] . "' AND (Status = 'Quote Accepted' OR Status = 'Quote Rejected')";
}else{
    $query = "SELECT * FROM users u INNER JOIN jobinfoweb j ON (u.customer = j.customer) WHERE u.userId = '" . $_SESSION['adminId'] . "' AND (Status = 'Quote Accepted' OR Status = 'Quote Rejected')";
}
...
if ($affect_rows > 0) {
    ...
    foreach ($datad as $row) {
        ...
        $job_jobno        = isset($row['Job']) ? $row['Job'] : "";
        $job_quote_date   = isset($row['QuoteJobDate']) ? $row['QuoteJobDate']: "";
        $job_quote_make   = isset($row['QuoteMake']) ? $row['QuoteMake']: "";
        $job_quote_model  = isset($row['QuoteModel']) ? $row['QuoteModel']: "";
        $job_quote_serial = isset($row['QuoteSerialNumber']) ? $row['QuoteSerialNumber']: "";
        $job_quote_amount = isset($row['QuoteTotal']) ? $row['QuoteTotal']: "";
        $job_quote_tax    = isset($row['QuoteTotalTax']) ? $row['QuoteTotalTax']: "";
        $job_quote_total  = isset($row['QuoteGrandTotal']) ? $row['QuoteGrandTotal']: "";
        $job_quote_status = isset($row['Status']) ? $row['Status']: "";
        $job_quote_accept = isset($row['AcceptQuote']) ? $row['AcceptQuote']: "";
        $job_quote_reject = isset($row['RejectQuote']) ? $row['RejectQuote']: "";
        $delivery_address = isset($row['DeliveryAddress']) ? $row['DeliveryAddress']: "";
        $workshop         = isset($row['Workshop']) ? $row['Workshop'] : "";
        ...
        $query2  = "SELECT * FROM JobUpdateWeb WHERE Job = '" . $row['Job'] . "' ";
        // echo "Query2: $query2 <br>";

        $db = IOC::make('database', array());

        //var_dump($db->selectquerys($query2));
        list($affect_rows, $query_data2) = $db->selectquerys($query2);
       
        
        $job_quote_accept = isset($query_data2[0]['AcceptQuote']) ? $query_data2[0]['AcceptQuote'] : 0;
