...
$conn = new PDO("mysql:host=192.168.2.24;port=3306;dbname=$db", $dbUser, $dbPassword);
...
function requestPassword($date,$time){ //30bb51
	...
	if($result->rowCount() >=1 )
	{
		...
		$update = "UPDATE users SET `userLoginPassword`=md5('".$newPass."')
 ...
 function login()
 {
  ...
  $result = $query->fetch(PDO::FETCH_ASSOC);
    ...
    {//login pass
        ...
        $sql = "UPDATE users SET userLastLogin=NOW(), userNoLogins='".mysql_real_escape_string($row['userNoLogins']+1)."' WHERE userId='".mysql_real_escape_string($row['userId'])."' ";
        $result = $conn->query($sql);
